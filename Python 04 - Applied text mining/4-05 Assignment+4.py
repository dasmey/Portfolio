...
def doc_to_synsets(doc):
    ...
    # get the wordnet from tags
    converted = [convert_tag(tag[1]) for tag in tags]
    # create a list (tokens, wordnet)
    pairs = list(zip(doc, converted))
    # get synsets for each pair
    sets = [wn.synsets(x,y) for x,y in pairs]
    # keep only the first synset
    answer = [s[0] for s in sets if len(s) > 0]
...
def similarity_score(s1, s2):
    ...
    for synset1 in s1:
        cuts = []
        for synset2 in s2:
            cuts.append(synset1.path_similarity(synset2))
        scores = [x for x in cuts if x is not None]
        if len(scores) > 0 :
...
def most_similar_docs():
    ...
    for i in range(len(paraphrases)):
        sim = document_path_similarity(paraphrases['D1'][i], paraphrases['D2'][i])
        if s < sim:
            s = sim
            result = (paraphrases['D1'][i], paraphrases['D2'][i], sim)
...
def lda_topics():
    ...
    answer = ldamodel.print_topics(num_topics=10, num_words=10)
